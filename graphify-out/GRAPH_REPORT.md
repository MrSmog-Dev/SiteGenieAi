# Graph Report - SiteGenieAi  (2026-07-09)

## Corpus Check
- 201 files · ~1,000,670 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1744 nodes · 2378 edges · 262 communities (119 shown, 143 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 295 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e120a815`
...
- Pydantic Models & Blueprint Service
- Ivy SEO/GEO Engine
...
- reportlab==4.2.5 dependency
- CLAUDE.md
- CLAUDE.md
- extraction-spec.md
- README.md
- Admin Test Credentials (admin@sitegenie.com)
- Google OAuth Test Flow (Mongo-seeded session)
- Graphify Skill Directive (.claude/CLAUDE.md)
- /graphify add <url>
- --watch folder auto-rebuild
- FalkorDB export
- graphify MCP stdio server
- Neo4j Cypher export
- Deterministic Node ID Format Rule (full-path stem + entity)
- Extraction Subagent Prompt Spec
- GitHub Clone + Cross-Repo Merge Flow
- Native CLAUDE.md Integration (graphify claude install)
- Post-commit auto-rebuild hook
- BFS/DFS Traversal Modes
- save-result Feedback Loop (work memory)
- Constrained Query Expansion Against Graph Vocabulary
- Whisper Domain-Hint Prompting from God Nodes
- --cluster-only Reclustering Flow
- --update Incremental Re-extraction Flow
- AST + Semantic Extraction Pipeline
- God Nodes
- GRAPH_REPORT.md audit report
- --update incremental rebuild
- Knowledge Graph Pipeline (code/docs/papers/images/video to graph)
- Atlas — data agent
- Blaze — social distribution agent
- Forge — template builder/curator agent
- Halo — support agent
- Ivy — SEO/GEO agent
- Ledger — finance agent
- Mara — email agent
- Persistent Agent Brain (agent_memory)
- Nova — strategy agent
- Quill — copywriting agent
- Rex — sales / lead-hunter agent
- Titan — COO / daily briefings
- Zephyr — growth agent
- AI Team — 12 autonomous agents (owner-only)
- Publish View Analytics Counter (views_total/views_daily)
- Hands-off Automation Loop (15-min tick)
- Credit-based Usage Model
- Custom Domain Mapping (DNS verify + host-based serving)
- Customer Feedback Inbox (owner)
- Functional Code Review Fixes (billing leak, indexes, capped arrays)
- "Waking the Genie" Outage Banner
- Halo Public Support Chat + Feedback Routing + Policy Pages
- True Inter-Agent Communication (memo bus)
- Ivy SEO Autopilot (keyword engine, GEO audit, calendar)
- LLM Call Timeout + Quality-to-Economy Fallback
- Launch Day Milestone Tracker
- Owner Bypass Role (unlimited credits)
- Exclusive Ownership Transfer + Certificate
- Failed-Payment Recovery
- AI Pricing Agent (code-depth metrics + Haiku analysis)
- Publish Flow (vanity slugs, SEO HTML, OG cards, ZIP export)
- Reset Business Data Owner Tool
- Rex Autopilot (auto-hunt, auto-demo, pitch drafts, demo-view alerts)
- Rex Lead Hunter (website scoring + Google Places hunt)
- Security Audit Fixes (CSP sandbox, SSRF guard, open-redirect validation)
- SiteGenie Product (website-template generator)
- Team Multi-seat (shared credit pool)
- Team Pulse (activity log + proactive pulse)
- Team Shared Brain
- Real Task Execution (team_tasks queue)
- Template Market (public /market storefront)
- In-app Top-Up Modal
- UI-UX-Pro-Max Skill + Conversational Build Canvas
- Visual Click-to-Edit Builder (Shopify-style)
- War Room (multi-agent meetings + decisions)
- Agent Memory Panel Tests
- Conversational Build Canvas Tests
- Customer Feedback Inbox Tests
- Halo Widget & Feedback Routing Tests
- Ivy SEO Autopilot API/UI Tests
- Ownership Onboarding + Certificate + Luxury Flow Tests
- Policy Pages Tests (FAQ/Refund/Terms/Privacy)
- Register Clickwrap Consent Tests
- Team Shared Brain Tests
- Testing Protocol (main/testing-agent communication format)
- War Room Decision Auto-Capture Tests

## God Nodes (most connected - your core abstractions)
1. `resolutions` - 42 edges
2. `The testing data must be entered in yaml format Below is the data structure:` - 42 edges
3. `_require_owner()` - 30 edges
4. `_call_llm()` - 30 edges
5. `log_activity()` - 26 edges
...
- `Architecture` --shares_data_with--> `emergentintegrations==0.2.0 dependency`  [INFERRED]
  memory/PRD.md → backend/requirements.txt
- `Architecture` --shares_data_with--> `fastapi==0.110.1 dependency`  [INFERRED]
  memory/PRD.md → backend/requirements.txt
- `Architecture` --shares_data_with--> `motor (async MongoDB driver) dependency`  [INFERRED]
  memory/PRD.md → backend/requirements.txt
- `Architecture` --shares_data_with--> `PyJWT==2.13.0 dependency`  [INFERRED]
  memory/PRD.md → backend/requirements.txt
- `Architecture` --shares_data_with--> `pymongo dependency`  [INFERRED]
  memory/PRD.md → backend/requirements.txt

## Import Cycles
- None detected.

## Communities (262 total, 143 thin omitted)
...
Cohesion: 0.08
Nodes (50): add_agent_memory(), add_team_memory(), agent_activity_feed(), agent_job_status(), agent_status_board(), clear_agent_chat(), clear_war_room(), delete_agent_memory() (+42 more)

### Community 2 - "Flagship Template Build Pipeline"
Cohesion: 0.09
Nodes (44): build_flagship(), _design_context(), _extract(), _inject(), main(), Builds SiteGenie's 3 flagship Market templates targeting the $500 Premium grade., _sonnet(), _upgrade() (+36 more)
...
Cohesion: 0.15
Nodes (26): google_session(), login(), logout(), me(), Request, Response, register(), generation_status() (+18 more)
...
Cohesion: 0.24
Nodes (13): Force stripe_sdk to talk directly to Stripe (not the emergent proxy).     Emerge, use_native_stripe(), cancel_subscription(), get_subscription(), Request, reactivate_subscription(), stripe_native_webhook(), subscription_checkout() (+5 more)

### Community 10 - "Team Management & Invites"
Cohesion: 0.11
Nodes (27): my_invites(), Team state for the Billing page (owner or member)., Pending invites addressed to me (to accept after signing up)., _require_team_owner(), team_accept(), team_account(), team_invite(), team_leave() (+19 more)
...
Cohesion: 0.06
Nodes (43): _count_view(), download_zip(), edit_template(), generate_template(), _inject_social_meta(), _normalize_domain(), og_card(), ownership_certificate() (+35 more)

### Community 13 - "Sample Generated Landing Page"
Cohesion: 0.25
Nodes (8): About section (brand story + stats), Contact form section, FAQ accordion section, Hero section (headline/subheadline/CTA), Services section (4 disciplines grid), Testimonials section, Verdant & Vine — sample generated garden-design landing page, Luxury High-Converting Generation Engine (upgraded prompts)

### Community 14 - "Pydantic Models & Blueprint Service"
Cohesion: 0.07
Nodes (50): AgentChatInput, AgentMemoryInput, BlueprintCreateInput, BlueprintSpec, BlueprintStatusInput, BlueprintUpdateInput, BuildMessageInput, CheckoutInput (+42 more)

### Community 15 - "Ivy SEO/GEO Engine"
Cohesion: 0.05
Nodes (95): start_war_room(), blog_index(), blog_post(), blog_sitemap(), _host(), Request, _eligible_pulse_agent(), log_activity() (+87 more)
...
Cohesion: 0.09
Nodes (33): delete_lead(), draft_outreach(), hunt(), hunt_status(), list_leads(), _require_owner(), scan(), update_lead() (+25 more)

### Community 18 - "Blog Engine Routes"
Cohesion: 0.18
Nodes (4): Seeds the Template Market with AI-generated, AI-priced templates. Run once: pyth, fulfill_market_purchase(), Exclusive sale: transfer full ownership of the site to the buyer, delist it from, datetime

### Community 19 - "Automation Loop (Outreach/Alerts)"
Cohesion: 0.05
Nodes (42): agent_communication:, -agent: "main"  # or "testing" or "user", -agent: "main"  # or "testing" or "user", -agent: "main"  # or "testing" or "user", backend:, -comment: "Detailed comment about status", -comment: "Detailed comment about status", created_by: "main_agent" (+34 more)
...
Cohesion: 0.06
Nodes (35): 1. Update Test Result File Before Testing:, 2. Incorporate User Feedback:, 3. Track Stuck Tasks:, 4. Provide Context to Testing Agent:, 5. Call the testing agent with specific instructions referring to test_result.md, - Add a message to `agent_communication` explaining what you've done, - Add implementation details to the status_history, and history, and is the primary means of communication between main and the testing agent. (+27 more)
...
Cohesion: 0.15
Nodes (18): ansi_ljust(), _detect_page_type(), format_ascii_box(), format_markdown(), format_page_override_md(), generate_design_system(), _generate_intelligent_overrides(), hex_to_ansi() (+10 more)
...
Cohesion: 0.21
Nodes (14): market_checkout(), market_preview(), market_purchase_status(), Request, checkout_status(), create_checkout(), Request, stripe_webhook() (+6 more)
...
Cohesion: 0.14
Nodes (9): get_milestones(), hash_password(), verify_password(), _initialize(), _run_db_init(), startup(), ensure_automation_state(), Periodically process active subscriptions for credit resets and renewals. (+1 more)
...
### Community 38 - "Website Blueprint API Tests"
Cohesion: 0.18
Nodes (4): Tests for the Website Blueprint API (/api/blueprints/*).  A Blueprint is standal, _register(), test_other_user_cannot_access(), user_session()
...
Cohesion: 0.13
Nodes (14): emergentintegrations==0.2.0 dependency, fastapi==0.110.1 dependency, motor (async MongoDB driver) dependency, PyJWT==2.13.0 dependency, pymongo dependency, Architecture, Backlog, Implemented (history) (+6 more)
...
Cohesion: 0.33
Nodes (5): Admin, API tests, Auth Testing Playbook (SiteGenie), Google flow (for browser test), Protected endpoints
...
Cohesion: 0.67
Nodes (3): litellm dependency, openai==1.99.9 dependency, 2-Pass Agentic Generation Pipeline (Quality Sonnet / Economy Haiku)
...
### Community 91 - "CRACO Webpack Config"
Cohesion: 0.22
Nodes (7): format_master_md(), persist_design_system(), Persist design system to design-system/<project>/ folder using Master + Override, Format design system as MASTER.md with hierarchical override logic., config, path, webpackConfig
...
- **Should `AI Team Agent Memory & Chat Routes` be split into smaller, more focused modules?**
  _Cohesion score 0.07918552036199095 - nodes in this community are weakly interconnected._
- **Should `Flagship Template Build Pipeline` be split into smaller, more focused modules?**
  _Cohesion score 0.08687943262411348 - nodes in this community are weakly interconnected._
- **Should `Backend Auth/Subscription Test Suite` be split into smaller, more focused modules?**
